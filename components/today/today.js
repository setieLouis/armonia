...
async function initToday() {
    ...
    await loadComponent('header-root', 'components/today/sub_components/header/header.html');

    // Step 3: Caricamento Calendario (che include la data)
    await loadComponent('calendar-root', 'components/today/sub_components/calendar/calendar.html');

    // Step 4: Caricamento Lista Pasti
    await loadComponent('meals-root', 'components/today/sub_components/meals/meals.html', async (element) => {
        // Carichiamo il JS specifico del componente se non è già presente
        if (typeof window.initMeals !== 'function') {
            await loadScript('components/today/sub_components/meals/meals.js');
        }
        window.initMeals(element);
    });

    // Step 5: Caricamento Progress bar
    await loadComponent('progress-root', 'components/today/sub_components/progress/progress.html');
}
